...
import React from "react";
import ArticleCard from "../../components/AriticleCard";
import { FiArrowRightCircle } from "react-icons/fi";
import { useQuery } from "@tanstack/react-query";
import { getallPosts } from "../../services/index/posts";
import toast from "react-hot-toast";
...
function Articles() {
  ...
  return (
    ...
    >
      ...
      <Link
        to={"/blog"}
        className="flex mx-auto text-primary px-6 py-2 rounded-lg items-center gap-x-3 font-semibold border-2 border-primary hover:text-white hover:bg-primary transition-all duration-500 ease-in-out"
      >
        <span>More articles</span>
        <FiArrowRightCircle className="h-7 w-7 p-0.5 rounded-full" />
      </Link>
